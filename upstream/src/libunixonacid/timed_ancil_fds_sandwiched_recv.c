...
int timed_ancil_fds_sandwiched_recv (int fdsock, int *fds, unsigned int n, char const *before, unsigned int beforelen, char const *after, unsigned int afterlen, struct taia const *deadline, struct taia *stamp)
{
  ...
  int received ;
  {
    ...
    uint32_unpack_big(gotbefore + beforelen, &sent) ;
    if (sent > n) return (errno = ERANGE, -1) ;
  }

  received = timed_ancil_fds_recv(fdsock, fds, sent, deadline, stamp) ;
  ...
  }
  {
    ...
    for (; i < (unsigned int)received ; i++)
      if (ndelay_on(fds[i]) < 0) goto err ;
  }
  return received ;
